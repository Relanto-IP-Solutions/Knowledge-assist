...
def _call_answer_generation(url: str, body: dict) -> dict:
    ...
    if not resp.ok:
        try:
            ...
            err_msg = (resp.text or resp.reason or "").strip() or str(resp.status_code)
        ...
        # Do not use raise_for_status(): its message omits the JSON body from Cloud Run.
        detail = err_msg if len(err_msg) <= 4000 else err_msg[:4000] + "…"
        raise RuntimeError(
            f"answer-generation HTTP {resp.status_code} at {url!r}: {detail}"
        ) from None
    return resp.json()
