...
def get_firebase_user(
    ...
    logger.bind(**log_ctx).info(
        "auth | lookup users row by firebase_uid (raw DB then ORM fallback if needed)"
    )
    tup = _lookup_user_tuple_by_firebase_uid(uid, db)
    ...
    # No row for this firebase_uid yet — not a DB outage; may link by email or 403 below.
...
def get_existing_firebase_user(
    ...
    t0 = time.perf_counter()
    logger.bind(**log_ctx).info(
        "auth strict | lookup users row by firebase_uid (raw DB then ORM fallback if needed)"
    )
    tup = _lookup_user_tuple_by_firebase_uid(uid, db)
    ms = int((time.perf_counter() - t0) * 1000)
    hit = bool(tup)
