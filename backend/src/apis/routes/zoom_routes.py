...
async def _list_recordings_for_master_oid(
    db: Session, oid: str, days_lookback: int
) -> list[tuple[str, list[dict[str, Any]]]]:
    """OID-strict mode: list recordings only for the configured master connector user."""
    normalized_oid = normalize_opportunity_oid(oid)
    opp = db.query(Opportunity).filter(Opportunity.opportunity_id == normalized_oid).first()
    if not opp:
        raise HTTPException(
    ...
    if not connector or not (connector.email or "").strip():
        raise HTTPException(
            status_code=400,
            detail=(
                "ZOOM_CONNECTOR_USER_EMAIL must point to an existing user email for "
                "master-only targeted Zoom discovery."
            ),
        )
    ...
    meetings = await client.list_recordings(
        from_date=from_d.isoformat(),
        to_date=to_d.isoformat(),
        user_id=connector.email.strip(),
    )
    for m in meetings:
        if isinstance(m, dict):
            m.setdefault("host_email", connector.email.strip())
    return [(connector.email.strip(), meetings)]
...
async def discover_zoom_recordings(
    ...
    try:
        # True Account-Wide S2S Search: Scan every user in the organization concurrently.
        logger.info("Zoom discover: Total Account scan. target_oid={}", target_oid or "All")
    ...
    for email, user_recordings in grouped_recordings:
        ...
        for rec in user_recordings:
            ...
            if not oid_token:
                ...
                continue
            oid = normalize_opportunity_oid(oid_token)
            if target_oid and oid != target_oid:
                continue
            user_matched += 1
            matched += 1
            if oid in seen_oids:
            ...
            # 100% Master-Only S2S: Use the connector account as the owner fallback.
            owner = connector
            if not opp:
                opp = Opportunity(
                    opportunity_id=oid,
...
@router.post("/integrations/zoom/connect/{oid}")
async def connect_zoom_for_opportunity(
    oid: str,
    db: Annotated[Session, Depends(get_db)],
):
    """Synchronous Zoom ingestion cycle: validate -> targeted discover -> ingest -> metrics."""
    try:
        normalized_oid = normalize_opportunity_oid(oid)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc

    if not _zoom_s2s_configured():
        raise HTTPException(
            status_code=400,
            detail=(
                "Zoom Server-to-Server OAuth is not configured. Set ZOOM_ACCOUNT_ID, "
    ...
    # Validate credentials by requesting an access token once.
    try:
        client = ZoomClient()
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Failed to validate Zoom OAuth configuration: {exc}",
        ) from exc

    # Targeted discovery for this OID (owner-focused, no account-wide scan).
    await discover_zoom_recordings(db=db, days_lookback=14, oid=normalized_oid)
    ...
    await ZoomSyncService().sync_opportunity(normalized_oid, db=db)
    # Keep existing metrics payload format unchanged.
    return zoom_metrics_for_opportunity(normalized_oid, db)
