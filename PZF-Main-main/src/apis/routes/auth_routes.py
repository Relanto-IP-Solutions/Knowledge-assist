...
async def google_url(
    ...
    try:
        # We pass provider as the 'state' so Google includes it in the callback URL.
        url = await oauth_service.get_google_auth_url(redirect_uri, provider=provider, state=provider)
...
@external_router.get("/auth/google/callback")
async def google_oauth_browser_callback(
    ...
    """Browser redirect endpoint for Google OAuth (GET ?code=)."""
    if error:
    ...
    try:
        result = await oauth_service.exchange_google_code(code, redirect_uri, db)
