...
def discover_gmail_threads(
    ...
    # NEW: Robust inbox scan fallback (for prefixes like 'oid200')
    if not thread_ids:
    ...
    if not thread_ids:
        logger.warning(
            "Gmail discover: all OID queries returned 0 threads for connector {} "
    ...
    return GmailDiscoverResponse(
        connector_user_email=connector.email,
        gmail_search_query="subject:oid (generic)",
...
def discover_gmail_threads_impl(
    ...
    return discover_gmail_threads(db=db, user_email=user_email)
...
def discover_gmail_threads_endpoint(
    ...
    """
    return discover_gmail_threads(db, user_email=user_email)
...
async def gmail_discover_start_integrations(
    ...
    """Start Gmail discovery flow; prompt OAuth only if Gmail scope is missing."""
    target_user, target_email = _resolve_gmail_identity(db, body.user_email)

    has_scope = bool(
        target_user and has_google_scopes(db, target_user.id, [_GMAIL_READONLY])
    )
    if has_scope:
        logger.info(
            "Gmail discover: direct-discover for user={} (scope already present).",
            target_email,
        )
        ...
        return {
            ...
            "message": "Gmail already authorized; discovery completed.",
    ...
    return {
        "requires_oauth": True,
        "auth_url": auth_url,
        "message": "Google OAuth required before discovery.",
    }
...
async def gmail_connect_integrations(
    ...
    """Start per-opportunity Gmail OAuth flow with signed state."""
    try:
    ...
    has_scope = bool(
        identity_user and has_google_scopes(db, identity_user.id, [_GMAIL_READONLY])
    )
    ...
    if has_scope:
        ...
        logger.info(
            "Gmail connect: already authorized; activating + starting sync for oid={} "
            "user_email={}",
            normalized_oid,
            target_email,
        )
        ...
        return {
            "requires_oauth": False,
            "message": "Already authorized for Gmail; sync started.",
            "oid": normalized_oid,
            "status": "ACTIVE",
            "sync_started": True,
        }
...
async def gmail_callback_integrations(
    ...
    try:
        ...
        # --- MODE: DISCOVER ---
        if mode == "discover":
            # Move discovery to background to avoid blocking the OAuth redirect (prevents "stuck" pages)
        ...
        # --- MODE: CONNECT ---
        normalized_oid = normalize_opportunity_oid(str(parsed_state["oid"]))
        opp = db.query(Opportunity).filter(Opportunity.opportunity_id == normalized_oid).first()
        if not opp:
