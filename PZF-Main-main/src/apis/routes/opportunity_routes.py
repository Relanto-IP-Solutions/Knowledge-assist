...
def _get_total_sase_questions_count_cached(db: Session | None = None) -> int:
...
class CreateOpportunityBody(BaseModel):
    opportunity_id: str = Field(
        ...,
        min_length=1,
        description="Business key (canonical format recommended, e.g. oid1234).",
    )
...
def create_opportunity(
    ...
    if "OWNER" not in roles_u:
        ...
        # Also update the request-scoped user object so downstream RBAC sees it.
        user.roles_assigned = db_user.roles_assigned

    existing = db.query(Opportunity).filter(Opportunity.opportunity_id == oid).first()
    if existing:
        raise HTTPException(status_code=409, detail="opportunity_id already exists.")
    ...
    # Best-effort: seed the RAG opportunities table (uses opportunity_id string).
    try:
    ...
    try:
        ...
        try:
            ...
            query = """
                ...
                ORDER BY b.batch_order, q.seq_in_section;
            """
            cur.execute(query)
            rows = cur.fetchall()
            ...
            if rows:
                ...
                cur.execute(
                    ...
                    FROM sase_picklist_options
                    WHERE q_id = ANY(%s)
                    ORDER BY q_id, sort_order
                    """,
                    (q_ids,),
                )
                for qid, opt in cur.fetchall():
                    option_by_qid.setdefault(qid, []).append(opt)
...
def _sase_question_pair(cur, qid: str) -> tuple[str | None, str | None]:
    """Load ``answer_type`` and ``requirement_type`` from ``sase_questions`` for one ``q_id``."""
    cur.execute(
        "SELECT answer_type, requirement_type FROM sase_questions WHERE q_id = %s",
        (qid,),
    )
    row = cur.fetchone()
    if not row:
...
def _calculate_opportunity_stats(cur, opp_id: str) -> dict[str, float | int]:
    ...
    total_questions = _get_total_sase_questions_count_cached()
...
def _submit_final_answer_selection(
    *,
    ...
    """
    effective_question_id = question_id
...
def load_questions(opportunity_id: str):
    ...
    try:
        ...
        questions = [dict(zip(q_cols, r, strict=True)) for r in q_rows]
...
def save_or_resolve_answers(
    ...
    **Auth:** ``Authorization: Bearer <Firebase ID token>`` (see dependency ``get_firebase_user``).
    ...
    def _assert_can_post_answers(cur, user_id: int, user_obj: User) -> None:
        ...
        # Resolve the canonical opportunity row and validate scope.
        cur.execute(
            """
    ...
    if q_updates:
        ...
        try:
            ...
            if not has_any_conflict:
                ...
                # Fallback: mixed payload with conflicts; keep per-item logic.
                for q_update in q_updates:
                    qid = str(q_update["q_id"])
                    ...
                    status_str = _coerce_answers_status(
                        q_update.get("status"), default="active"
                    )
                    is_user_override = bool(q_update.get("is_user_override", False))
                    override_text = _normalize_optional_str(q_update.get("override_value"))
                    if is_user_override and not override_text:
                    ...
                    effective_qid = _submit_final_answer_selection(
                        cur=cur,
                        now=now,
                        opportunity_id=opportunity_id,
                        question_id=qid,
                        ...
                        is_user_override=is_user_override,
                        override_text=override_text,
                        set_final_answer_id=False,
                    )
                    touched_qids.append(effective_qid)
                    final_answer_updates.append(
                        (opportunity_id, effective_qid, selected_answer_id)
                    )

                    feedback_id = _normalize_optional_str(q_update.get("feedback_id"))
                    feedback_type = q_update.get("feedback_type")
                    comments = _normalize_optional_str(q_update.get("comments"))
                    if feedback_id and feedback_type is not None:
                        pending_feedback.append(
                            {
                    ...
                    results.append(
                        {
                            "question_id": effective_qid,
                            "answer_id": selected_answer_id,
                            "final_answer_id": selected_answer_id,
                            "conflict_id": conflict_id,
                            "answer_type": None,
                            "requirement_type": None,
                            "feedback_id": feedback_id,
    ...
    try:
        ...
        if action == "RESOLVE":
            ...
            con.commit()
            ans_type, req_type = _sase_question_pair(cur, qid)
