...
from src.services.plugins.gmail_plugin import (
    _build_thread_json,
    _get_credentials,
    _gmail_sync_query_list,
)
...
def _sync_opportunity_impl(oid: str) -> dict[str, Any]:
    ...
    with Session(get_engine()) as db:
        ...
        for q in queries:
            list_resp = (
                service.users().threads().list(userId="me", q=q, maxResults=50).execute()
            )
            batch = list_resp.get("threads") or []
            if batch:
                threads = batch
                logger.info(
                    "GmailSyncService: matched query={!r} thread_count={} oid={}",
                    q,
                    len(batch),
                    normalized,
                )
        ...
        if not threads:
            logger.warning(
                "GmailSyncService: no threads for oid={} (tried {} queries)",
                normalized,
                len(queries),
            )
